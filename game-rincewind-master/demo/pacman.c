...
typedef struct state {
  size_t time;
  scene_t *scene;
  size_t time_since_last_key_press;
} state_t;
 ...
 */
void create_pacman(scene_t *scene, double radius) {
  list_t *shape =
  ...
  *first = (vector_t){radius, 0};
  ...
  body_t *pacman = body_init(shape, DEFAULT_MASS, PACMAN_COLOR);
